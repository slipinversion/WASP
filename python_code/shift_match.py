...
def shift_match(data_type, plot=False, method='full'):
    ...
    plot_folder = 'tele_shift' if data_type == 'tele' else 'strong_shift'\
        if data_type == 'strong' else 'cgps_shift'
    if not os.path.isdir(plot_folder):
        os.mkdir(plot_folder)
    synthetics_file = 'synm.tele' if data_type == 'tele' else 'synm.str'\
        if data_type == 'strong' else 'synm.cgps'
...
#            continue
#        if not file['name'] == 'FDF':
#            continue
        print(file['name'], file['component'])
        synt_tr = file['synthetic']
        stream = read(file['file'])
        nshift = int(2 / dt) if data_type == 'tele' else int(2 / dt)\
            if data_type == 'strong' else int(4 / dt)
        ...
        if plot:
            ...
            observed1 = np.array(
                [val for i, val in enumerate(obser_tr[start + tr_shift:])\
                 if i < length])
...
if __name__ == '__main__':
    ...
    parser.add_argument("-t", "--type", choices=['tele', 'strong', 'cgps'],
